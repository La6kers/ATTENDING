        ...
        foreach (var prop in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            if (!ReferenceEquals(original, sanitized) && original != sanitized)
            {
                ...
                else
                {
                    ...
                    {
                        // Fail closed: if we cannot sanitize a property that contains
                        // potentially dangerous content, refuse to process the request.
                        throw new InvalidOperationException(
                            $"Cannot sanitize init-only property {prop.Name} on {type.Name}. Use a settable property or apply validation separately.");
                    }
                }
            }
        }
