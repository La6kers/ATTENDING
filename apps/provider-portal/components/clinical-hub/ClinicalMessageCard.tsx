    ...
    <div className={cn(
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <div className="text-xs text-gray-500">{getTypeIcon(message.type)}</div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2">
              ...
              {message.newDiagnosis && (
                <span className="text-xs px-1.5 py-0.5 rounded font-semibold bg-red-100 text-red-800">
                  NEW DX
                </span>
              )}
          ...
          <div className="flex gap-1 mt-1">
            <span className="text-[11px] px-1.5 py-0.5 rounded font-semibold bg-green-100 text-green-700">
              AI Analysis Ready
            </span>
            {message.clinicalGuidelines && (
              <span className="text-[11px] px-1.5 py-0.5 rounded font-semibold bg-gray-100 text-gray-600">
                Guidelines Available
              </span>
            )}
            {message.priority > 6 && (
              <span className="text-[11px] px-1.5 py-0.5 rounded font-semibold bg-red-100 text-red-700">
                Needs Review
              </span>
            )}
          </div>
    ...
    <div className="border-t border-gray-200 bg-gray-50 p-3 space-y-3">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        ...
        {(message.criticalFactors || message.vitals) && (
          <div className="flex gap-3 mt-2 pt-2 border-t border-blue-200 text-xs text-blue-700">
      ...
      <div className="grid grid-cols-4 gap-2 p-2 bg-white rounded">
        <div className="flex flex-col gap-0.5">
          <div className="text-[11px] text-gray-500 uppercase font-semibold">Age/Gender</div>
          <div className="text-[11px] font-semibold text-gray-900">{message.age}{message.gender}</div>
        </div>
        {message.pmh && (
          <div className="flex flex-col gap-0.5">
            <div className="text-[11px] text-gray-500 uppercase font-semibold">PMH</div>
        ...
        {message.medications && Array.isArray(message.medications) && typeof message.medications[0] === 'string' && (
          <div className="flex flex-col gap-0.5">
            <div className="text-[11px] text-gray-500 uppercase font-semibold">Current Meds</div>
            <div className="text-xs font-semibold text-gray-900">{message.medications.length} active</div>
          </div>
        )}
      </div>
      ...
      {message.clinicalGuidelines && (
        <div className="bg-amber-50 border border-amber-200 rounded p-2">
          ...
          {message.clinicalGuidelines.map((guideline, idx) => (
            <div key={idx} className="text-xs text-amber-900 mb-0.5 pl-3 relative">
          ...
          <div className="grid grid-cols-2 gap-2">
            {message.recommendedActions.map((action, idx) => (
              ...
              >
                ...
                <div className="text-[11px] opacity-90">Priority: {action.priority}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 mb-2 max-h-32 overflow-y-auto">
          {getActionOptions(activeTab, message).map((option, idx) => (
            <button
              ...
              className="p-1.5 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded text-xs text-center transition-colors"
            >
              {option.label}
            </button>
          ))}
