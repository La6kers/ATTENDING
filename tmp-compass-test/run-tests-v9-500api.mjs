...
// Tests layman language normalization accuracy
// IDs 700-1199, mix of peds/adult/geri, ED/UC/PC
// ============================================================
...
function buildCases() {
  ...
  // Target distribution: 125 peds, 200 adult, 175 geri
  const demoSlots = [
    ...Array(125).fill('peds'),
    ...Array(200).fill('adult'),
    ...Array(175).fill('geri'),
  ];
  // Shuffle
...
// --- Match function (from stress test prompt) ---
function isHit(r) {
...
async function main() {
  ...
  let md = `# COMPASS Stress Test — 500 API Cases (v9)\n\n`;
  ...
  writeFileSync('tmp-compass-test/results-v9.md', md);
  writeFileSync('tmp-compass-test/results-v9.json', JSON.stringify(results, null, 2));

  console.log(`\n=== RESULTS ===`);
  console.log(`Accuracy: ${accuracy}% (${hits}/${total})`);
  console.log(`Misses: ${misses} | Errors: ${errors}`);
  console.log(`Latency: p50=${p50}ms p95=${p95}ms p99=${p99}ms`);
  console.log(`\nReports written to tmp-compass-test/results-v9.md and results-v9.json`);
}
