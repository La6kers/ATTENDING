...
const DELAY_MS = 200; // Dev mode allows 1000/min — fast iteration
...
const MATCH_ALIASES = {
  ...
  'esophageal foreign body': ['button battery ingestion', 'foreign body', 'gi foreign body'],
  'cannabinoid hyperemesis syndrome': ['chs', 'cannabis hyperemesis'],
  'bipolar disorder — manic episode': ['bipolar', 'mania', 'manic'],
  'hand foot and mouth disease': ['hfm', 'hfmd', 'coxsackie', 'viral exanthem'],
  'intimate partner violence': ['ipv', 'domestic violence', 'physical abuse'],
  'acute urinary retention': ['urinary retention', 'bph'],
  'inguinal hernia': ['hernia'],
  'plantar fasciitis': ['heel pain'],
  'erectile dysfunction': ['ed', 'impotence'],
  'serotonin syndrome': ['serotoninergic'],
  'neuroleptic malignant syndrome': ['nms'],
  'hepatic encephalopathy': ['toxic/metabolic encephalopathy'],
  'delirium tremens': ['dts'],
  'slipped capital femoral epiphysis': ['scfe', 'hip fracture'],
  'henoch-schonlein purpura': ['iga vasculitis', 'hsp'],
  'carbon monoxide poisoning': ['co poisoning', 'co exposure'],
  'near drowning / submersion injury': ['near drowning', 'drowning', 'submersion'],
  'herpes zoster (shingles)': ['shingles', 'herpes zoster', 'zoster'],
  'tinea corporis (ringworm)': ['ringworm', 'tinea'],
};
...
async function main() {
  ...
  let md = `# COMPASS Stress Test — 600 API Cases (v21)\n\n`;
  ...
  writeFileSync('tmp-compass-test/results-v21.md', md);
  writeFileSync('tmp-compass-test/results-v21.json', JSON.stringify(results, null, 2));

  console.log(`\n=== RESULTS ===`);
  console.log(`Accuracy: ${accuracy}% (${hits}/${total})`);
  console.log(`Misses: ${misses} | Errors: ${errors}`);
  console.log(`Latency: p50=${p50}ms p95=${p95}ms p99=${p99}ms`);
  console.log(`\nReports written to tmp-compass-test/results-v21.md and results-v21.json`);
}
